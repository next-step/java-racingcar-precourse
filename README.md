...
- 세 개의 요구사항을 만족하기 위해 노력한다. **특히 기능을 구현하기 전에 기능 목록을 만들고, 기능 단위로 `commit`하는 방식으로 진행한다.**
  - :white_check_mark: 주어진 횟수 동안 n대의 자동차는 전진 또는 멈출 수 있다.
  - :white_check_mark: 각 자동차에 이름을 부여할 수 있다. 전진하는 자동차를 출력할 때 자동차 이름을 같이 출력한다.
  - :white_check_mark: 자동차 이름은 쉼표(,)를 기준으로 구분하며 이름은 5자 이하만 가능하다.
  - :white_check_mark: 사용자는 몇번의 이동을 할것인지를 입력할 수 있어야 한다.
  - :white_check_mark: 전진하는 조건은 0에서 9사이에서 `random`값을 구한후, `random`값이 4이상일 경우 전진하고, 3이하의 값이면 멈춘다.
  - :white_check_mark: 자동차 경주 게임을 완료한 후 누가 우승했는지를 알려준다.
  - :white_check_mark: 우승자가 한명 이상일 경우, 쉼표(,)로 이름을 구분해 출력한다.
  - :white_check_mark: 사용자가 잘못된 값을 입력할 경우 `[ERROR]`로 시작하는 에러 메시지를 출력 후 입력을 다시 받는다.

## 기능 구현 목록

- Name: 자동차 이름 래퍼 클래스
  - 5자 이하의 값을 가진다.
  - `null` 또는 공백을 허용할 수 없다.
- Position: 자동차 위치 래퍼 클래스
  - 0 이상의 값을 가진다.
  - 전진된 값을 반환한다.
  - 다른 위치와 비교하여 큰 위치를 반환한다.
- Car: 자동차 도메인
  - 전진 규칙에 따라 전진하거나 멈춘다.
- Cars: 자동차 일급 컬렉션
  - 자동차 이름은 중복될 수 없다.
  - 자동차들을 경주시킨다.
  - 관리하고 있는 자동차 이름을 반환한다.
- PositionCarsGroup: 같은 거리끼리 자동차들을 그룹화한 일급 컬렉션
  - 자동차들의 거리중 최대 거리를 반환한다.
  - 특정 위치에 해당되는 자동차들을 반환한다.
- RaceCount: 경주 횟수 래퍼 클래스
  - 최소 0이상의 값이어야 한다.
  - 경주 횟수를 감소 시킬 수 있다.
  - 남아 있는 경주 횟수가 있는지 판단할 수 있다.
- Round: 주어진 게임 횟수 동안 자동차들의 경주 시킬 도메인
  - 게임 횟수를 관리한다.
    - 다음 게임 라운드를 진행할 수 있는지 판단한다.
  - 라운드에 자동차들을 경주한다.
    - 각각 자동차들은 규칙에 따라 전진 또는 멈춘다.
- RandomForwardMoveRule: 자동차 전진 규칙 함수
  - `0 ~ 9` 사이의 랜덤 값이 생성한다.
  - 규칙
    - 4이상의 값이면 전진한다.
    - 3이하의 값이면 멈춘다.
- Winner: 우승한 자동차를 관리하는 일급 컬렉션
