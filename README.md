...
* Validator : Operator(컨트롤러)가 사용자로부터 경주 할 자동자 이름을 입력받을 때, 이름이 5자 이하인지 검사한다.
  ...
  * 시도할 회수를 입력받을 때, 숫자가 아니라면 Operator(컨트롤러)에게 메시지를 보낸다.
* Operator(컨트롤러) : 
  * Validator가 자동자 이름을 입력받을 때, 중복된 이름이 존재하거나, 이름이 5자 초과인 경우라고 알려주면,  
  "[ERROR]"로 시작하는 에러 메시지를 출력 후, 'IllegalArgumentException'을 발생시키고, 그 부분부터 입력을 다시 받는다.
  * Validator가 자동자 경주 게임을 몇 번 실행할 지, 시도할 회수를 입력받을 때, 숫자가 아니라고 알려주면, 
  "[ERROR]"로 시작하는 에러 메시지를 출력 후, 'IllegalArgumentException'을 발생시키고, 그 부분부터 입력을 다시 받는다.
