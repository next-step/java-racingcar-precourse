...
### 내가 생각한 문제...
```
각 클래스의 역할이 명확하지 않았다.. 특히 util 클래스의 역할이 무엇인가...!! @_@
그러다보니 여기저기의 메서드를 불러와 의존도가 엄청 높았다
```

## 따라서 클래스의 역할을 더 명확히 해보자!!
### Controller
유저에게 정보를 입력 받아서, 게임을 실행한다.
### RacingGame
게임에 참가하는 자동차들을 기억하고, 컨트롤러가 게임을 실행하라고 할 때 실행한다. 
### UserInterface
게임 셋팅에 필요한 정보들을 입력하고 결과를 확인한다.
### Car
랜덤하게 달리는 기능을 구현하고, 거리를 움직일 수 있다.
### CarFactory
일반카, 슈퍼카들을 제공한다. 여기서 이름에 유효한 값을 검사하지 않는 이유는 위에서 다 했으니까..? 그런데 에러처리를 여기서 해야하는게 맞나 싶기도 하고..
애러처리를 하는 지점에 대해서 고민을 해봐야할 것 같다..ㅜㅜ
### RacingGameFactory 
RacingGame의 라운드횟수가 바뀌지 않길 바랬다. 그래서 상수로 하고 싶었고 따라서 생성자에서 초기화를 해야할 필요가 있었다.
또한 RacingGame의 생성을 최대한 유효한 값이 확정이 났을때로 뒤로 미루고 싶었다...!!
그래서 따로 RacingGame의 생성을 책임지고, 라운드 횟수와 이름들이 모두 유효한 값임이 판정이 났을때!! 생성하도록 실행했다.

처음에는 이런 부분을 컨트롤러에게 맡길까 생각해봤는데, 컨트롤러가 게임횟수를 내부 변수로 아는 건 너무 이상하게 느껴졌다..
그리고 이 횟수가 바뀌질 않길 정말정말 안전하게 보관하길 원해서 이런 코드를 구현했다.

대신 이 값이 정말 유효할때만 생성되게 하기 위해서 생성하는 메서드에 예외처리를 추가했다..!
