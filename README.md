...
- Name: 자동차 이름
  ...
  - 자동차 이름은 null이거나 공백이면 안된다.
- Position: 자동차의 위치
  - 최소 0 이상의 위치를 가진다.
  - 전진된 값을 반환한다.
  - 다른 위치 값을 비교하여 큰 위치를 반환한다.
- Car: 자동차
  - 자동차는 전진 규칙에 따라 앞으로 나아갈 수 있다.
  - 문자열 이름을 관리하는 Name 타입의 지역변수를 가지고 있다.
  - 위치를 의미하는 Position 타입의 지역변수를 가지고 있다.
    - 몇번 움직였느냐에 대한 데이터
- Cars: 자동차 일급 컬렉션
  - 중복된 이름을 가진 자동차가 있는지 체크한다.
  - 자동차들을 경주시킨다.
  - 자동차들 이름을 반환한다.
- PositionCarsGroup: 거리끼리 그룹화하여 Map 타입을 지닌 일급 컬렉션
  - 자동차들의 거리중 최대 거리를 반환한다.
  - 특정 거리에 해당되는 자동차들을 반환한다.
- RaceCount: 경주 횟수
  - 최소 0이상의 값이어야 한다.
  - 경주 횟수를 감소 시킬 수 있다.
  - 남아 있는 경주 횟수가 있는지 판단할 수 있다.
- Round: 게임 횟수별 자동차들의 상태를 관리할 도메인
  - 게임 횟수를 관리한다.
    - 다음 게임 라운드를 진행할 수 있는지 판단해야 된다.
  - 해당 라운드에 자동차들을 경주한다.
    - 각각 자동차들은 규칙에 따라 전진한다.
- GameRule: 자동차 전진 규칙
  - 4이상의 값이면 전진한다.
  - 3이하의 값이면 멈춘다.
