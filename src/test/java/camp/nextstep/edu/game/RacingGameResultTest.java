...
class RacingGameResultTest {
    ...
    @DisplayName("게임 우승자 결과 반환 - 성공 테스트")
    void winnerList_size_successTest(int number1, int number2, int number3, int winnerCount) {
        ...
        // when
        RacingGameResult gameResult = new RacingGameResult(racingCarList);

        // then
        assertThat(gameResult.getWinnerList().size()).isEqualTo(winnerCount);
    }
}
