...
1. 게임의 한 턴이 진행되면 전진 혹은 멈추는 자동차를 구현한다.
  * 1) 자동차 역할을 하는 Car 클래스를 구현한다. 
       Car 클래스의 전진 기능을 하는 forward() 함수를 구현하고, 멈춤 기능은 forward() 함수를 호출하지 않음으로써 달성한다.
       forward() 함수는 Car 클래스의 위치값을 1 증가시키고, 증가된 위치값을 리턴한다.
  * 2) 랜덤 값을 생성하여 Car 클래스의 forward() 함수에게 전달하는 RandomCarMover 클래스를 구현한다.
2. 게임의 한 턴 동안 모든 자동차들을 한번씩 전진 시도하는 기능을 구현한다.
  * 1) 한 턴의 게임을 진행하는 GameTurn 클래스를 구현하고, run() 함수를 호출하여 실제 한 턴을 수행한다.
3. 게임의 턴을 여러번 진행하는 기능을 구현한다.
  * 1) 주어진 횟수 만큼 GameTurn의 run() 함수를 호출하는 GameTurns 클래스를 추가한다.
4. 게임의 모든 턴이 끝나고 최종 우승자를 선별하는 기능을 구현한다.
  * 1) 게임의 결과를 토대로 우승자를 가려내는 RacingGame 클래스를 추가한다.
  * 2) 자동차 목록을 관리하는 Cars 클래스에 위치값이 가장 큰 자동차들을 선별하는 기능을 추가한다.
5. 사용자에게 게임에 참가할 플레이어들의 이름을 입력받는 기능을 구현한다.
6. 사용자에게 게임의 총 턴수를 입력받는 기능을 구현한다.
7. 게임의 한 턴이 끝날 때마다 게임의 진행 상황을 Console 창에 표시하는 기능을 구현한다.
8. 게임이 끝나고 최종 우승자들이 선별되면 Console 창에 표시하는 기능을 구현한다.
9. 사용자가 플레이어들의 이름을 잘못 입력한 경우 에러 메시지를 표시하고 재입력 받는 기능을 구현한다.
10. 사용자가 게임의 총 턴수를 잘못 입력한 경우 에러 메시지를 표시하고 재입력 받는 기능을 구현한다.
11. 게임이 진행되면서 우승자 목록을 관리하는 기능을 추가한다.
  * 1) WinnerPicker 클래스를 추가하여 게임의 매 턴마다 자동차의 움직임을 감지한 후, 현재 우승자 목록을 갱신한다.
12. MVC 패턴에 맞추어 프로그램을 개발한다.
  * 1) TDD 방식으로 Business Logic 부분인 Model 패키지를 먼저 개발한다.
  * 2) Model 패키지가 어느 정도 완성되면 사용자 입력을 받기 위해 View 패키지, Controller 패키지를 구현한다.
